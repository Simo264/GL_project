...
static array<string, 6> skyboxImages = {
  "res/Skybox/back.jpg",
...
SkyBox::SkyBox()
{
  ...
  {
    u_char* data = stbi_load(skyboxImages[i].c_str(), &width, &height, &nrChannels, 0);
    if (data)
    {
    ...
    {
      spdlog::error("Cubemap tex failed to load at path: {}", skyboxImages[i]);
    }
